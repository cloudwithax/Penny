...
public class RemoveMoneyCommand implements CommandExecutor {
    ...
    public RemoveMoneyCommand(Plugin plugin) {
        this.plugin = plugin;
    }
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender != null) {
            if (args.length > 0) {
                ...
                plugin.getDatabase().removeMoney(target, amount);
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&7You have removed &a$" + amount + " &7from &a" + target.getDisplayName() + "'s &7balance"));
                target.sendMessage(ChatColor.translateAlternateColorCodes('&', "&a" + player.getDisplayName() + " &7has removed &a$" + amount + " &7from your balance"));
